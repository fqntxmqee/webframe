...
/** Sets a <code>java.sql.Types.BIGINT</code> on a query using <code>PreparedStatement.setLong()</code>.
 * Conversion is provided from <code>java.lang.Long</code>. Conversion from a string is provided using method <code>Long.parseLong()</code>.
 * 
...
public class LongSetter extends AbstractSetter
{
   ...
   public int set(PreparedStatement query, int index, Object value) throws SQLException, ParseException
   {
      ...
      {
         query.setNull(index++, Types.BIGINT);
      }
      else
      {
         throw new IllegalArgumentException("Cannot convert value of class " + value.getClass().getName() + " to long at position " + index);
      }
      return index;
   }
}